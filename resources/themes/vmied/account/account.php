    ...
    <div class="offcanvas-body p-0 d-flex flex-column">
        ...
        <div class="flex-grow-1 p-2 overflow-auto">
            <div class="list-group list-group-flush" hx-boost="true" hx-target="#app-content" hx-select="#app-content" hx-swap="outerHTML show:window:top" >
                ...
                <?php if (isset($user->type_id) && in_array((int)$user->type_id, [1, 2], true)): ?>
                <a href="<?= (int)$user->type_id === 1 ? '/admin/vips' : '/app/members' ?>" hx-boost="false" onclick="bootstrap.Offcanvas.getInstance(document.getElementById('profileDrawer')).hide();" class="list-group-item list-group-item-action border-0 rounded-3 py-3 px-3 d-flex align-items-center gap-3 text-secondary fw-medium hover-bg-light">
                    <i data-lucide="users" style="width: 18px;"></i> <?= (int)$user->type_id === 1 ? 'Quản lý VIP' : 'Quản lý thành viên' ?>
                </a>
                <?php endif; ?>
            </div>
        </div>
